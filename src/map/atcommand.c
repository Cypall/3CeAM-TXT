...
ACMD_FUNC(mapmove)
{
  ...
  if(!atcmd_checktime(sd,battle_config.warp_delay) && !pc_isGM(sd)) { // Command Delay by Vengeance
    sprintf(atcmd_output, msg_txt(801), battle_config.warp_delay);
    clif_displaymessage(fd,atcmd_output);
      return 0;
  }
      atcmd_savetime(sd);
...
ACMD_FUNC(go)
{
    ...
    if(!atcmd_checktime(sd,battle_config.go_delay) && !pc_isGM(sd)) {// Command Delay by Vengeance
  sprintf(atcmd_output, msg_txt(802), battle_config.go_delay);
  clif_displaymessage(fd,atcmd_output);
    return 0;
  }
    atcmd_savetime(sd);
