...
int pc_split_atoui(char* str, unsigned int* val, char sep, int max)
{
  ...
  int atcmd_savetime(struct map_session_data* sd)
  {
    time_t timer;
  struct tm *t;
    ...
    pc_setglobalreg(sd, "PC_LAST_ATCMD_TIME", t->tm_mday*24*60*60 + t->tm_hour*60*60 + t->tm_min*60 + t->tm_sec);    
  return 0;
  }
  
  int atcmd_checktime(struct map_session_data* sd,int time_tick)
  {
  int diff;
    time_t timer;
  struct tm *t;
    ...
    diff = t->tm_mday*24*60*60 + t->tm_hour*60*60 + t->tm_min*60 + t->tm_sec - pc_readglobalreg(sd, "PC_LAST_ATCMD_TIME");
   
  return !(diff >= 0 && diff < time_tick);
  }
